...
workspace_id = "1fa42805-42c8-49e3-ad4d-f54fbd1a4f57" # Check workspace prod
dataset_id = "bbf2b0e7-3c45-40c6-949a-4799ab1d0a98" # Check semantic model prod
...
if df.status[0] == "Unknown":
    logger.info("Semantic Model: Erajaya Dashboard - Dataset is refreshing right now. Please wait until this refresh has finished to trigger a new one.")
elif df.status[0] == "Disabled":
    logger.error("Semantic Model: Erajaya Dashboard - Dataset refresh is disabled. Please enable it.")
elif df.status[0] == "Failed":
    logger.warning("Semantic Model: Erajaya Dashboard - Last Dataset refresh failed. Please check error message.")
elif df.status[0] == "Completed":
    api_call = requests.post(url=url, headers=header)
    logger.info("Semantic Model: Erajaya Dashboard - Triggered a Dataset refresh")
else:
    logger.warning("Not familiar with status, please check documentation for status: '" + df.status[0] + "'")